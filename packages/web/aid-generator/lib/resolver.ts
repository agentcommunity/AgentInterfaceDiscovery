...
import { AidManifest, AuthConfig, UserConfigurableItem, AuthPlacement, CertificateConfig, RequiredPathItem } from '@aid/core';
...
export async function* resolveDomain(domain: string): AsyncGenerator<ResolutionStep> {
    ...
    try {
        ...
        const response = await fetch(`/api/proxy?url=${encodeURIComponent(`https://cloudflare-dns.com/dns-query?name=${recordName}&type=TXT`)}`, {
